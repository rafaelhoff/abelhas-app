...
export class PhotoLibraryModalPage {
  ...
  async fetchPhotos() {
    ...
    try {
      ...
      if (!photolib) {
        this.logger.warn('IonicNative PhotoLibrary: not available with this Device');
        return false;
      }

      photolib.subscribe((res: any) => {
        this.logger.log('Library: ' + JSON.stringify(res));
        res.library.forEach(libraryItem => {
          ...
          this.logger.log(JSON.stringify(libraryItem, null, '\t'));
        });
      ...
      this.logger.log('permissions weren\'t granted');
    }

    return;

    // this.photoLibrary.getLibrary(null, null,
    //  { thumbnailWidth: THUMBNAIL_WIDTH, thumbnailHeight: THUMBNAIL_HEIGHT/*, chunkTimeSec: 0.3*/ }).subscribe({
    //   next: (chunk) => {
